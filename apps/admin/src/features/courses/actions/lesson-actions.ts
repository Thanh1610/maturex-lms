...
export async function getLessonUploadUrlAction(input: {
  ...
  try {
    ...
    if (token) {
      ...
      if (payload?.sub && (!payload.role || ["admin", "superadmin", "instructor", "tutor"].includes(payload.role))) {
        isAdmin = true;
      }
    }
    ...
    const allowedPrefixes = ["video/", "application/x-mpegURL", "application/vnd.apple.mpegurl"];
    ...
    if (!isVideo && !isVideoExt) {
      return {
        success: false,
        error: "Định dạng tệp không được hỗ trợ. Chỉ chấp nhận các file video (.mp4, .webm, .mov).",
      };
    }
    ...
    const msg = error instanceof Error ? error.message : "Không thể tạo URL tải lên.";
...
export async function createLessonAction(input: {
  ...
  try {
    ...
    return { success: false, error: "Không thể tạo bài học. Vui lòng thử lại." };
  }
}
...
export async function updateLessonAction(input: {
  ...
  try {
    ...
    return { success: false, error: "Không thể cập nhật bài học. Vui lòng thử lại." };
  }
}
...
export async function deleteLessonAction(input: {
  ...
  try {
    ...
    return { success: false, error: "Không thể xóa bài học. Vui lòng thử lại." };
  }
}
