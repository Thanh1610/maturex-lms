...
async function main() {
    ...
    if (a.person === "me") {
      ...
      await prisma.assignment.upsert({
        ...
        create: {
          id: a.id,
          userId: admin.id,
          courseId: a.courseId || (a as any).course,
          title: a.title,
          description: (a as any).description || "",
          due: a.due,
          type: a.type,
  ...
  for (const s of skillSeeds) {
    await prisma.userSkill.upsert({
      ...
      update: {
        level: (s as any).level ?? (s as any).current ?? 0,
        target: s.target,
        name: s.name,
        group: (s as any).group ?? (s as any).category ?? "Chuyên môn",
        color: (s as any).color ?? "lavender",
      },
      create: {
        userId: admin.id,
        skillKey: s.id,
