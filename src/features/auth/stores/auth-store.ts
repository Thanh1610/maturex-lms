...
import type { SanitizedUser } from "../services/auth-service";
...
export interface AuthState {
  ...
  logout: () => Promise<void>;
  fetchMe: () => Promise<SanitizedUser | null>;
  ...
  persist(
    (set) => ({
      ...
      login: async (email: string, password: string) => {
        ...
        try {
          const res = await api<{ user: SanitizedUser }>(
      ...
      fetchMe: async () => {
        set({ isLoading: true });
        try {
          const res = await api<{ user: SanitizedUser | null }>(
            API_ROUTES.auth.me,
          );
          set({ user: res.user, isLoading: false, error: null });
          return res.user;
          ...
          try {
            const refreshRes = await api<{ user: SanitizedUser }>(
