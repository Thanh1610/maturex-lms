...
import { isCourseInstructor } from "./course-access.js";
...
export function initOrganization(db) {
...
function permittedUsers(db, user) {
  check(
...
function courseProgress(db, userId, course) {
  const total = db
...
function pathProgress(db, id, userId) {
...
function listPaths(db, user) {
  return db
    ...
    .map((path) => {
      ...
      return {
        ...path,
        courses: pathCourses(db, path.id),
...
function savePath(db, user, body, id) {
  teach(user);
  return transaction(db, () => {
    if (id) {
      ...
      if (body.status === "published")
        check(
          pathCourses(db, id).every((course) => course.status === "published"),
          409,
      ...
      check(
        Array.isArray(body.course_ids) &&
          body.course_ids.length > 0 &&
          body.course_ids.length <= 50 &&
          body.course_ids.every((x) => typeof x === "string") &&
          new Set(body.course_ids).size === body.course_ids.length,
        400,
      ...
      ).run(id, user.id, title, description, now());
      body.course_ids.forEach((courseId, position) =>
        db
          .prepare("INSERT INTO path_courses VALUES (?,?,?)")
          .run(id, courseId, position),
      );
    }
    return {
      ...db.prepare("SELECT * FROM learning_paths WHERE id=?").get(id),
      courses: pathCourses(db, id),
    };
  });
}

function joinPath(db, user, id, body, assign) {
  if (assign) manage(user);
  const userId = assign ? textField(body.user_id, "Người học", 100) : user.id;
  let reason = "",
    dueDate = null;
  if (assign) {
    check(
...
function reportData(db, user) {
  ...
  if (user.role === "admin" || user.role === "instructor")
    for (const course of db
      ...
      .all()
      .filter((course) => isCourseInstructor(db, user, course.id)))
  ...
  for (const row of rows) {
    ...
    const summary = courseMap.get(row.course_id);
    summary.enrolled++;
    summary[row.status]++;
  }
  const paths = new Map();
  for (const member of members)
    for (const path of member.paths) {
      ...
      const summary = paths.get(path.id);
      summary.enrolled++;
      if (path.status === "completed") summary.completed++;
...
export function handleOrganization({
  ...
  const match = /^\/paths\/([^/]+)(?:\/(enroll|assign))?$/.exec(path);
